...
Figure \ref{fig:langc} describes the language of \langc, with its type rules defined in \ref{fig:langc-type} excluding common type rules in \textit{PCF}. Values are basic number, booleans, and functions. Expressions are the usual forms of CPCF, with an extra expression $\cMon{k}{l}{j}{\kappa}{e}$ that guards the expression $e$ with a contract $\kappa$. A contract can be a flat contract $\cFlat{c}$, which can only be applied to a value, and verify the value against function $c$; a function contract $\cKFunc{\kappa_1}{\kappa_2}$, which guards the input arguments with a domain contract $\kappa_1$ and guards the output/return value with a range contract $\kappa_2$. Dependent contract $\cKDep{\kappa_1}{x: τ} {\kappa_2}$ behaves similarly to a function contract but the range contract can use the input value.
...
This thesis introduces two languages. To effectively distinguish the languages used, we have chosen different typeset styling for both languages. \langc\ will be typeset in \attrC{blue\ san-serif}. The reduction arrow of \langc\ is $\cStep$.
...
\begin{figure}[h]
  ...
  \begin{tabular}{llll}
    ...
    Expressions & $\attrC{e}$      & $::=$ & $\attrC{0, 1, \dots} \mid \attrC{tt} \mid \attrC{ff} \mid \attrC{x} \mid \cLam{x:\tau}{e} \mid \attrC{\mu x:\tau. e}$ \\
                  &          & $\mid$ & $\attrC{e_1 + e_2} \mid \attrC{e_1 \wedge e_2} \mid \attrC{e_1 \vee e_2} \mid \attrC{e_1 \ e_2}$ \\
                  &          & $\mid$ & $\cIf{e_1}{e_2}{e_3} \mid \attrC{zero?(e)}$ \\
                  &          & $\mid$ & $\cMon{k}{l}{j}{\kappa}{e} \mid \cErr{l}{p}$ \\
    Values      & $\attrC{v}$      & $::=$ & $\attrC{0, 1, \dots} \mid \attrC{tt} \mid \attrC{ff} \mid \cLam{x}{e} \mid \cErr{l}{p}$ \\
    Contexts    & $\attrC{E}$      & $::=$ & $\attrC{[\cdot]} \mid \attrC{E + e} \mid \attrC{v + E} \mid \attrC{E \wedge e} \mid \attrC{v \wedge E} \mid \attrC{E \vee e} \mid \attrC{v \vee E}$ \\
