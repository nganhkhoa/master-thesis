...
Figure \ref{fig:langc} describes the language of \langc, with its type rules defined in \ref{fig:langc-type} excluding common type rules in \textit{PCF}. Values are basic number, booleans, and functions. Expressions are the usual forms of CPCF, with an extra expression $\mon{k}{l}{j}{\kappa}{e}$ that guards the expression $e$ with a contract $\kappa$. A contract can be a flat contract $\Kflat{c}$, which can only be applied to a value, and verify the value against function $c$; a function contract $\Karrow{\kappa_1}{\kappa_2}$, which guards the input arguments with a domain contract $\kappa_1$ and guards the output/return value with a range contract $\kappa_2$. Dependent contract $\kappa_1 \xrightarrow{d} (λ x: τ. \kappa_2)$ behaves similarly to a function contract but the range contract can use the input value.

Reduction rules for \langc\ is provided in Figure \ref{fig:langc-reduction}. A flat contract monitor expression reduces into an if expression checking if the value meets the contract predicate. Else, the entire program reduces to an error. A function contract monitor expression reduces into a function where its argument are wrapped with the domain contract, and the application result is wrapped with the range contract. Argument contract labels are changed for blame correctness. Dependent contract is similarly reduces like a function contract, but the range contract has the input value wrapped up with the domain contract with different labels set. The labels set used in dependent contract is by the indy semantics, which has been proven about its correctness \cite{completemonitors}. To distinguish from the reduction of \lange\ modeling the Effect Handlers, reduction in \langc\ is denoted using $\step$.
...
\begin{figure}[h]
  ...
  \begin{itemize}
      \item[] \begin{tabular}{llll}
          Basic Types & $o$ & $::=$ & $\Tnum \mid \Tbool$ \\
          Types & $\tau$ & $::=$ & $o \mid \Tarrow{\tau_1}{\tau_2} \mid \Tcon{\tau}$ \\
          ...
          Values & $v$ & $::=$ & $0, 1, ... \mid \ttval \mid \ffval \mid λ x. e \mid \error{l}{p}$ \\
          Contexts & $E$ & $::=$ & $\hole \mid E + e \mid v + E \mid E \wedge e \mid v \wedge E \mid E \vee e \mid v \vee E$ \\
          & & $\mid$ & $E \ e \mid v \ E \mid \text{if } E \text{ then } e \text{ else } e \mid \zeroq{E}$ \\
          & & $\mid$ & $\mon{k}{l}{j}{\kappa}{E}$
      \end{tabular}
  \end{itemize}
...
\begin{figure}[h]
  ...
  \end{mathpar}
  \caption{\langc\ Type Rules}
  \label{fig:langc-type}
\end{figure}
