...
The next section illustrates many examples aiming to give an intuition to the
...
only one operation in the handler $\attrE{h_3}$. If the argument is a function, then
$\attrE{h_3}$ should capture all possible effects, collectable after compiling
recursively with known contract.


\begin{figure}
\begin{align*}
  \cMon{k}{l}{j}{\cKFunc{(\cKFunc{\cFlat{\kappa_1}}{\cFlat{\kappa_2}})}{\cFlat{\kappa_3}}}{f}
\end{align*}
$\Downarrow$
\begin{align*}
  \cLam{g}{\cMon{k}{l}{j}{\cFlat{\kappa_3}}{f \ (\cMon{l}{k}{j}{\cKFunc{\cFlat{\kappa_1}}{\cFlat{\kappa_2}}}{g})}}
\end{align*}
$\Downarrow$
\begin{align*}
  \cLam{g}{\cMon{k}{l}{j}{\cFlat{k_3}}{f \ (\cLam{x}{\cMon{l}{k}{j}{\cFlat{\kappa_2}}{g \ (\cMon{k}{l}{j}{\cFlat{\kappa_1}}{x})}})}}
\end{align*}
$\Downarrow$

\begin{align*}
  % 1. Define the handlers with their correct blame polarities
  \attrE{h_1} \ &\triangleq \ \attrE{\{} \attrE{check_{\kappa_1} \to} \dots \eBlame{k} \attrE{\}} \\
  \attrE{h_2} \ &\triangleq \ \attrE{\{} \attrE{check_{\kappa_2} \to} \dots \eBlame{l} \attrE{\}} \\
  \attrE{h_3} \ &\triangleq \ \attrE{\{} \attrE{check_{\kappa_3} \to} \dots \eBlame{k} \attrE{\}} \\[2ex]
  % 2. Build the nested evaluations bottom-up
  \attrE{e_1} \ &\triangleq \ {\ePerform{check_{\kappa_1}}{\tau_1} \ \attrE{x}} \\
  \attrE{e_2} \ &\triangleq \ {\ePerform{check_{\kappa_2}}{\tau_2} \ (\attrE{g} \ \attrE{e_1})} \\
  \attrE{e_3} \ &\triangleq \ {\ePerform{check_{\kappa_3}}{\tau_3} \ (\attrE{f} \ (\eLam{x}{\attrE{e_2}}))} \\[2ex]
...
\begin{figure}
\begin{align*}
  \cMon{k}{l}{j}{\cKDep{\cFlat{\kappa_1}}{x}{\cFlat{k_2}}}{g}
\end{align*}

$\Downarrow$

\begin{align*}
  \cLam{x}{\cMon{k}{l}{j}{\cSubst{\cMon{l}{j}{j}{\cFlat{\kappa_1}}{x}}{x}{\cFlat{\kappa_2}}}{g \ (\cMon{l}{k}{j}{\cFlat{\kappa_1}}{x})}}
\end{align*}
...
\begin{align*}
  % 1. Define the handlers with their correct blame polarities
  \attrE{h_1} \ &\triangleq \ \attrE{\{} \attrE{check_{\kappa_1} \to} \dots \eBlame{l} \attrE{\}} \\
  \attrE{h_2} \ &\triangleq \ \attrE{\{} \attrE{check_{\kappa_2} \to} \\
  &\qquad \eTAbs{\alpha}{
    \eLam{y : \alpha}{
      \eLam{kont : \alpha \attrE{\to \langle\rangle} \alpha}{
        \eIf{\kappa_2[x:=x_{dep}]}
            {\attrE{kont \ y}}
            {\eBlame{k}}
      }
    }
  } \\
  \attrE{h_3} \ &\triangleq \ \attrE{\{} \attrE{check'_{\kappa_1} \to} \dots \eBlame{l} \attrE{\}} \\[2ex]
  % 2. Build the nested evaluations bottom-up
  \attrE{x_{arg}} \ &\triangleq \ {\ePerform{check_{\kappa_1}}{\tau_1} \ \attrE{x}}\\
  \attrE{x_{dep}} \ &\triangleq \ {\eHandle{h_3}{(\ePerform{check'_{\kappa_1}}{\tau_1} \ \attrE{x})}}\\
  \attrE{e_1}     \ &\triangleq \ {\ePerform{check_{\kappa_2}}{\tau_2} \ {(\attrE{g} \ \attrE{x_{arg}})}} \\[2ex]
  % 3. The final compiled expression
  &\eLam{x}{\attrE{e_1}}
\end{align*}
...
The compilation is recursive, types are used to form well-typed \lange\
expressions. We define the following theorems for our compiler: Type
Preservation, Effect Safe Compiler, Observable Equivalance, and Blame
Preservation. Type Preservation Theorem states that a well-typed program in
\langc\ compiles into a well-typed program in \lange\ typed using the same
basic type and has some effects. Effect Safe Compiler Theorem makes sure that
all (contract) effects are handled-able using the compiler outputed handler.
Observable Equivalance Theorem and Blame Preservation Theorem both ensure the
same semantic behavior is preserved whether the program executed correctly or a
blame occured.

Because \lange\ has different types from \langc, we define $\symtt{T}$ as the
type conversion from \cGamma\ to \eGamma. The conversion is simple because
\langc\ only has basic type, so we map to the basic type of \lange.

In Figure \ref{fig:compilation}, we provide some interesting compilation rules,
other expressions left out should be trivial. Types used in \lange\ that copied
from \langc context are assumed to be converted.
...
\begin{theorem}[Type Preservation]
  If $\cCompile{\cGamma}{e}{\tau}{h}{e'}$
  then $\eTypesC{\eGamma}{e'}{\tau^*}{\epsilon}$
  for some $\attrE{\epsilon}$ and $\symtt{T}(\cGamma) \subseteq \eGamma$
\end{theorem}

\begin{theorem}[Effect Safe Compiler]
  If $\cCompile{\cGamma}{e}{\tau}{h}{e'}$ and $\symtt{T}(\cGamma) \subseteq \eGamma$
  then $\eTypesC{\eGamma}{\eHandle{h}{e'}}{\tau^*}{\langle\rangle}$
\end{theorem}

\begin{theorem}[Observable Equivalance]
  If $\attrC{e} \cStepMany \attrC{v}$ and $\cCompile{\cdot}{e}{\tau}{h}{e'}$ then there
  exists a $\attrE{v'}$ such that $\eHandle{h}{e'}\eStepMany \attrE{v'}$
\end{theorem}

\begin{theorem}[Blame Preservation]
  If $\attrC{e} \cStepMany \cErr{l}{p}$ and $\cCompile{\cdot}{e}{\tau}{h}{e'}$\\
  then $\eHandle{h}{e'}\eStepMany \eBlame{p}$
\end{theorem}
...
\begin{figure}[htbp]
  ...
  \begin{align*}
    \symtt{wrap}(\attrE{h},\attrE{e}) \ &\triangleq \ \eHandle{h}{e} \quad (\attrE{e} \neq \eLam{x}{e'}) \\
    \symtt{wrap}(\attrE{h},\eLam{x}{e}) \ &\triangleq \ \eLam{x}{\textcolor{black}{\symtt{wrap}(}h,e\textcolor{black}{)}}
  \end{align*}
