  ...
  stack(
    rule-set(
      prooftree(rule(
        ...
        $compile(type(Gamma,mon(k,l,j,flat(e_1),e_2),tau), apply(perform(effcheck, tau: tau), tuple(k, tuple(e_3, e_4))))$,
      )),
      prooftree(rule(
        name: [Compile-Func],
        ...
        $e_3 = e_1[x:=e_2]$,
        $compile(type(Gamma,mon(k,l,j,kappa_1->kappa_2,e),tau_1 -> tau_2),apply((lambda f: tau_1 -> tau_2. lambda x: tau_1. f),e_3))$,
      )),
      prooftree(rule(
        name: [Compile-Dep],
